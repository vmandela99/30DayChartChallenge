...
crop_prod_df <- read_csv("../data/fao_stat_data.csv")
...
crop_production <- crop_prod_df %>%
  select(Area, Item, Year, Value) %>% 
  pivot_wider(names_from = Item, values_from = Value) %>% 
  filter(Year > 2020, Area == c("Colombia", "Kenya", "United States of America", "China")) %>% 
  mutate(Code = case_when(Area == "Colombia" ~ "Co",
                          Area == "Kenya" ~ "Ke",
                          Area == "China" ~ "Cn", 
                          Area == "United States of America" ~ "Us",
                          TRUE ~ "Others")) %>% 
  pivot_longer(cols = -c(Area, Code,Year )) %>% 
  rename(Entity = Area) 

crop_selection <- crop_production %>% 
...
# already done name is crop_production
...
crop_production_sum <- crop_production %>% 
  group_by(Year, name) %>% 
  summarise(total = sum(value, na.rm = TRUE)) %>%
  ungroup()
...
crop_lines_label <- crop_production_sum %>% 
  filter(Year == max(Year)) %>%
  arrange(total) %>%
  mutate(cum = cumsum(total),
         posy = lag(cum, default = 0) + total / 2) %>%
...
year_list <- tibble(year = seq(1960, 2022, 10))
...
crop_order <- crop_production_sum %>% 
  filter(Year == max(Year)) %>%
  arrange(desc(total)) %>%
  pull(name)
...
color_crop <- c()
color_crop[crop_order] <- colorRampPalette(c("purple", "#b40059"))(7)
...
crop_production_species_plt <- crop_production_sum %>%
  mutate(name = fct_relevel(name, crop_order)) %>%
  ggplot() +
  geom_segment(data = axis_p1, aes(x = x, xend = xend, y = y, yend = yend), color = "white", linetype = "13", size = 0.3, inherit.aes = FALSE) +
  geom_text(data = axis_p1, aes(x = x, y = yend, label = comma(yend)), color = "white", hjust = 0, nudge_y = 2e6, size = 2.5, family = "roboto", inherit.aes = FALSE) +
  geom_text(data = year_list, aes(x = year, y = -3.5e6, label = year), color = "white", size = 2.5, family = "roboto", inherit.aes = TRUE)+
  geom_area(aes(Year, total, fill = name), color = "grey60") +
  geom_segment(data = crop_lines_label, aes(x = 2022, xend = 2025, y = posy, yend = posyend, color = name))+
  geom_segment(data = crop_lines_label, aes(x = 2030, xend = 2050, y = posyend, yend = posyend, color = name))+
  geom_label(data = crop_lines_label, aes(x = 2025, y = posyend, label = name, color = name), fill = bck_color, size = 3, family = "khula", nudge_y = 3e6, label.size = 0, hjust = 0, fontface = "bold", inherit.aes = FALSE) +
    geom_label(data = lines_label, aes(x = 2025, y = posyend, label = glue("{comma(total)} tonnes"), color = fish), fill = bck_color, size = 3, family = "khula", nudge_y = -2e6, label.size = 0, hjust = 0, fontface = "bold", inherit.aes = FALSE) +
  annotate("text", x = 1960, y = 150e6, label = "Evolution by species", family = "khula", size = 5, color = "white", hjust = 0) +
  annotate("text", x = 2040, y = 150e6, label = "Distribution among the 4 largest producers for year 2013", family = "khula", size = 5, color = "white", hjust = 0) +
  annotate("text", x = 1960, y = 143e6, label = "(tonnes)", family = "roboto", size = 3, color = "white", hjust = 0) +
...
ragg::agg_png(here::here("2021/2021-42-global_seafood/", paste0("global_seafood_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".png")), res = 320, width = 14, height = 7, units = "in")
